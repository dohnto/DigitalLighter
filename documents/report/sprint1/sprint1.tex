...
We learned that we want zeroconf software written in Java with similar set of services as Bonjour.
...
For the core communication and organization of our product we selected Client-Server Architecture figure \ref{fig:sprint1_arhitecture}.
...
Figure \ref{fig:class_diagram_server} gives an overview of the server class structure and collaboration. We made separate thread "ServerThread" for incoming clients to optimize server. Sending command signals is also moved from UI thread to "SendingThread" in order to make application more responsive.
...
Actual burn down chart can be seen on figure \ref{fig:Burn1 } that is generated by team  management tool targetProcess 3 \ref{targetProcessToolDescription}.  
