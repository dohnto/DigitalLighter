...
In this chapter there will be explained how the plan to test the product, who will be responsible for it, and very short mention the test criteria.
...
The customer did not want to focus on the testing part in this project, but in order to deliver a valid product there is a need to do some testing. 
In order to save time and spend most of the working hours on the implementation, black box testing will be used.  


The \emph{happy path} approach testing was accepted.
A happy path is a default scenario featuring no exceptional or error conditions, and comprises the sequence of activities executed if everything goes as expected. 
...
This means that the time is not spent on writing tests, on the other hand there is 
This means that we do not spend time actually writing tests, but still we have to manually test that the product does what it is supposed to.

Verification and validation testing is carried out continually. 
