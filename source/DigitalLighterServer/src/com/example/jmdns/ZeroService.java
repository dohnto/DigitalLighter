...
public abstract class ZeroService {
	 ...
	 * The suitable service instance. After API Level 15, Android supports the NSD in SDK, so use it instead
	 * of the JmnDNS solution.
	 * 
	 * @return The NsService instance.
	 */
	public static ZeroService create(Context conext, boolean host, boolean wifiEnable, InetAddress addr,
			String name) {
		/*
		 * if (android.os.Build.VERSION.SDK_INT < android.os.Build.VERSION_CODES.JELLY_BEAN) { return new
		 * JmDNSService(conext, host, wifiEnable, addr, name); } else { return new NsdService(conext, addr); }
		 */
