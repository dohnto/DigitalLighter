...
import com.silentducks.digitallighter.MainActivity;
import com.silentducks.netlib.NetUtil;
import com.silentducks.netlib.dns.DNSMessage;

import android.net.wifi.WifiManager.MulticastLock;
import android.util.Log;

/**
 * This thread runs in the background while the user has our program in the foreground, and handles sending
 * mDNS queries and processing incoming mDNS packets.
 * 
 * @author simmons
 */
public class NetThread extends Thread {
	...
	public NetThread(MainActivity mainActivity) {
		...
		netUtil = new NetUtil(mainActivity);
	}
	...
	/**
	 * The main network loop. Multicast DNS packets are received, processed, and sent to the UI.
	 * 
	 * This loop may be interrupted by closing the multicastSocket, at which time any commands in the
	 * commandQueue will be processed.
	 */
	@Override
	public void run() {
		...
		while (true) {
			...
			/*
			 * Log.v(TAG, String.format("received: offset=0x%04X (%d) length=0x%04X (%d)",
			 * response.getOffset(), response.getOffset(), response.getLength(), response.getLength()));
